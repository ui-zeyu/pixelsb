...
from pixelsb.domain.formatting import binary_bit_index, format_delta, format_sample
...
def test_format_delta_keeps_a_sign_and_zero_is_plain() -> None:
    assert format_delta(3, 8, DisplayFormat.DECIMAL) == "+3"
    assert format_delta(-1, 8, DisplayFormat.DECIMAL) == "-1"
    assert format_delta(0, 8, DisplayFormat.DECIMAL) == "0"
    assert format_delta(-10, 8, DisplayFormat.HEX) == "-0A"
    assert format_delta(-1, 8, DisplayFormat.BINARY) == "-00000001"
    assert format_delta(255, 8, DisplayFormat.HEX) == "+FF"
...
def test_format_rejects_values_outside_the_bit_depth() -> None:
    ...
    with pytest.raises(ValueError):
        format_delta(256, 8, DisplayFormat.DECIMAL)
